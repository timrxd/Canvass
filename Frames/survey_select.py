...
class SelectSurvey(Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        # Row 3 Headers
        self.top_bar.grid_columnconfigure(0, weight=6)
        self.top_bar.grid_columnconfigure(1, weight=1)
        self.top_bar.grid_columnconfigure(2, weight=1)
        self.top_bar.grid_columnconfigure(3, weight=4)
        self.top_bar.grid_columnconfigure(4, weight=1)

        h_row = 3
        self.doc_name_label = Label(self.top_bar, text="Survey Name", bg="grey", relief=RAISED, anchor="w")
        self.doc_name_label.grid(row=h_row, column=0, sticky="nswe")

        self.last_mod = Label(self.top_bar, text="Last Modified", bg="grey", relief=RAISED, anchor="w")
        self.last_mod.grid(row=h_row, column=1, sticky="nswe")

        self.responses = Label(self.top_bar, text="Responses", bg="grey", relief=RAISED, anchor="w")
        self.responses.grid(row=h_row, column=2, sticky="nswe")

        self.location = Label(self.top_bar, text="Location", bg="grey", relief=RAISED, anchor="w")
        self.location.grid(row=h_row, column=3, sticky="nswe")

        self.date = Label(self.top_bar, text="Event Date", bg="grey", relief=RAISED, anchor="w")
        self.date.grid(row=h_row, column=4, sticky="nswe")

        # Survey Table
        self.survey_table = Frame(self, bg="white")
        self.survey_table.pack(fill=BOTH, expand=TRUE)

        self.survey_table.grid_columnconfigure(0, weight=6)
        self.survey_table.grid_columnconfigure(1, weight=1)
        self.survey_table.grid_columnconfigure(2, weight=1)
        self.survey_table.grid_columnconfigure(3, weight=4)
